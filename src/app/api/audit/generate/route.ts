...
export async function POST(request: NextRequest) {
  try {
    ...
    // Generate 10 LinkedIn posts using OpenAI
    const posts = await generateLinkedInPosts(contentSample, name);
...
async function generateLinkedInPosts(content: string, authorName?: string): Promise<any[]> {
  if (!OPENAI_API_KEY) {
    // Return mock posts if no API key
    return getMockPosts();
  }
...
- Mix of formats: stories, insights, listicles, hot takes, how-tos
...
Generate 10 LinkedIn posts from the provided content. Each post should:
...
Return JSON array with objects containing: format, hook (first line), body (full post), hashtags (array of 3-5).`
          },
          {
            role: "user",
            content: `Generate 10 LinkedIn posts from this content:\n\n${content.substring(0, 6000)}`
          }
        ],
        temperature: 0.8,
...
This is exactly what Agent→Agent does automatically.
